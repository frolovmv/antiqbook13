  ...
  begin
    ...
    try
      ...
      Button1.Caption := 'Отсоединить';
       idFtp1.ChangeDir('htdocs/f_catalog/data');
...
procedure TForm17.IdFTP1Status(axSender: TObject; const axStatus: TIdStatus;
  const asStatusText: String);
begin
  lsbStatus.ItemIndex := lsbStatus.Items.Add(asStatustext)
end;

procedure TForm17.IdLogDebug1LogItem(ASender: TComponent;
  var AText: String);
begin
  lsbStatus.ItemIndex := lsbStatus.Items.Add(AText)
end;

procedure TForm17.cbShowStatusClick(Sender: TObject);
begin
...
function TForm17.GetFileName(S: String): String;
